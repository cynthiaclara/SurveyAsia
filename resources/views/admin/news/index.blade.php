              ...
              @foreach ($news as $item)
                <tr style="background-color: #F7FAFC;">
                  <td scope="col" class="py-4 ps-3">
                    <h6 class="fw-bold" style="color: #2A4365">
                      {{ $item->title }}</h6>
                    <span class="d-block text-secondary"
                      style="font-size: 13px">Posted
                      {{ $item->created_at->diffForHumans() }}</span>
                  </td>
                  <td scope="col">
                    <div class="text-published rounded-pill text-center w-75">
                      Published</div>
                  </td>
                  <td scope="col">
                  ...
                  <td scope="col" class="text-end pe-3">
                    ...
                    <ul class="dropdown-menu"
                      aria-labelledby="dropdown-manage-news">
                      <li><a class="dropdown-item" href="#"
                          data-bs-toggle="modal" data-bs-target="#modal-edit-news"
                          data-title="{{ $item->title }}"
                          data-description="{{ $item->description }}"
                          data-img="{{ $item->img }}"
                          data-id="{{ $item->id }}">Edit File</a></li>
                      <li>
                        <form
      ...
      <div class="modal-content">
        <div class="modal-body py-4">
          ...
          <button type="submit" class="btn text-white mx-auto px-lg-3 mt-5"
            style="background-color: #4C6FFF">Simpan</button>
